...
def process_seq_vector(indirs, side="left"):
    for indir in indirs:
        ...
        print(f"Loaded {num_imgs} images and {len(tss_imgs)} timestamps for {indir}")
        ...
        f.close()
        print(f"Finished writing poses1 for {indir}")
        ...
        for q in quatlist_us:
            f.write(f"{q[0]} {q[1]} {q[2]} {q[3]} {q[4]} {q[5]} {q[6]} {q[7]}\n")
        f.close()
        print(f"Finished writing poses2 for {indir}")
        continue
        ...
        print(f"Loaded calibration files for {side} camera.")
        # undist images
