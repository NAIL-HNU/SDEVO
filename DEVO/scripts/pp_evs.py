...
import h5py
import sys
sys.path.insert(0, '/home/njk/ros/SDEVO/src/sdevo/DEVO/utils')
from bag_utils import read_H_W_from_bag, read_tss_us_from_rosbag, read_images_from_rosbag, read_evs_from_rosbag_and_write, read_calib_from_bag, read_t0us_evs_from_rosbag, read_poses_from_rosbag
import scipy
...
def process_dirs(indirs, side="left", DELTA_MS=None):
    for indir in indirs: 
        ...
        if seq != "simulation_3planes":
            if side == "left":
                # intrinsics = [434.1778, 433.8335, 328.6492, 232.6556, 
                #             0.04334948, -0.06016529, 0, 0]
                # evs
                # intrinsics = [475.836648902014,476.370880076768,315.584512988753,242.063425344727,
                            # -0.0935648, 0.194458, 7.64243e-05, 0.00195639]
                # dvxplorer
                intrinsics = [677.407223846027,677.407642704641,337.24089744339,228.993010070381,
                            -0.072056461554079,-0.264611361175809, 0, 0]
            else:
                intrinsics = [434.1872, 434.0652, 324.3261, 239.8637,
                            0.05184187, -0.07847694, 0, 0]
            fx, fy, cx, cy, k1, k2, p1, p2 = intrinsics
            Kdist =  np.zeros((3,3))   
            Kdist[0,0] = fx
            Kdist[0,2] = cx
        ...
        #                                 [0.0, 0.0, 0.0, 1.0]])
        # else:
        #     posetopic = f"/dvxplorer_{side}/pose"
        ...
        read_evs_from_rosbag_and_write(bag, topics[evtopic_idx], f, H=H, W=W)
        f.close()
