    ...
    torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> u, const int t0)
{
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    const float d = (Z >= 0.02 & Z < 2.0) ? 1.0 / Z : 0.0; 
    const float d2 = d * d;
...
__global__ void reprojection_residuals_and_hessian_lr(
    const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> poses,
    const torch::PackedTensorAccessor32<float,4,torch::RestrictPtrTraits> patches,
    const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> intrinsics,
    const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> target,
    ...
    torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> u, const int t0)
{
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    {
      const float r = target[n][0] - x1;
      const float w = mask * weight[n][1] * 2.0;
...
std::vector<torch::Tensor> cuda_ba(
    ...
    torch::Tensor jj, 
    torch::Tensor kk,
    torch::Tensor poses_l2r,
    torch::Tensor target_l2r,
    torch::Tensor weight_l2r,
    torch::Tensor ii_lr,
    torch::Tensor jj_lr, 
    torch::Tensor kk_lr,
    const int t0, const int t1, const int iterations)
{
  ...
  auto ktuple_lr = torch::_unique(kk_lr, true, true);
  torch::Tensor kx_lr = std::get<0>(ktuple_lr);
  torch::Tensor ku_lr = std::get<1>(ktuple_lr);
  ...
  poses_l2r = poses_l2r.view({-1, 7});
  
  torch::Tensor kk_max_value = torch::max(kk);
  torch::Tensor kk_min_value = torch::min(kk);

  torch::Tensor ii_max_value = torch::max(ii);
  torch::Tensor ii_min_value = torch::min(ii);

  //   // 打印最大值
  // std::cout << "kk Max value: " << kk_max_value.item<int>() << " kk Min value: " << kk_min_value.item<int>() << std::endl;
  // std::cout << "kk size: " << kk.size(0) << std::endl;

  // std::cout <<"ii Max value: " << ii_max_value.item<int>() << " ii Min value: " << ii_min_value.item<int>() << std::endl;
  // std::cout << "ii size: " << ii.size(0) << std::endl;
  // std::cout <<"pose_l2r: " << poses[0][0] << " " << poses[0][1] << " " << poses[0][2] << " " << poses[0][3] << " " << poses[0][4] << " " << poses[0][5] << " " << poses[0][6] << std::endl;
  ...
  target_l2r = target_l2r.view({-1, 2});
  weight_l2r = weight_l2r.view({-1, 2});
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    reprojection_residuals_and_hessian_lr<<<NUM_BLOCKS(ii_lr.size(0)), NUM_THREADS>>>(
      poses_l2r.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      patches.packed_accessor32<float,4,torch::RestrictPtrTraits>(),
      intrinsics.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      target_l2r.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      weight_l2r.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      lmbda.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
      ii_lr.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
      jj_lr.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
      kk_lr.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
      ku_lr.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
      B.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      E.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
      C.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
      v.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
