...
def transform_l2r(poses, patches, intrinsics, kk):
    """ projective transform """
    ii = torch.zeros_like(kk)
    # backproject
    X0 = iproj(patches[:,kk], intrinsics[:,ii]) # (b,n_patches,h_patch,w_patch,4), where last dim (X,Y,1,1/Z)
    # print("X0: ", X0[0,0,0,0,:])
    # print("X01: ", X0[0,1,0,0,:])
    # print("kk0: ", kk[0,:])
    # print("kk1: ", kk[1,:])
    # transform
    # poses (b,n,7)
    Glr = poses
    X1 = Glr[:,:,None,None] * X0 # (b,n_patches,h_patch,w_patch,4), where last dim (X,Y,Z,1)
    p = X1.shape[2]

    # print("intrinsics_l: ", intrinsics_l.shape)
    # print("intrinsics_r: ", intrinsics_r.shape)
    # project
    x1 = proj(X1, intrinsics[:,ii]) # (b,n_patches,h_patch,w_patch,2), where last dim (x,y)
