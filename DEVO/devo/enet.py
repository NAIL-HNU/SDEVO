...
class Patchifier(nn.Module):
    ...
    def fmap_only(self, images):
        """ extract patches from input images """
        fmap = self.fnet(images) / 4.0 # (1, 15, 128, 120, 160)

        # useless but necessary
        x = torch.randint(1, 2, size=[1, 1], device="cuda")
        y = torch.randint(1, 2, size=[1, 1], device="cuda")
    ...
    def forward(self, images, patches_per_image=80, disps=None, return_color=False, scorer_eval_mode="multi", scorer_eval_use_grid=True):
        ...
        # print("t_temp1: ", time.time() - t_begin)
        t0 = time.time() # (1, 15, 128, 120, 160)
        ...
        time_gi = time.time()
        # print("used time se: ", time_gi - time_se)
        ...
        # print("used time gi:", time.time() - time_gi)
        ...
        # print("t_temp2: ", t_patch)
        if self.training:
            if self.patch_selector == SelectionMethod.SCORER:
                return fmap, gmap, imap, patches, index, scores
        else:
            if return_color:
                return fmap, gmap, imap, patches, index, clr, t_patch
            
        return fmap, gmap, imap, patches, index, t_patch
