...
def evaluate(config, args, net, train_step=None, datapath="", split_file=None,
    ...
    dataset_name = "hnu_evs"
    assert side == "left" or side == "right"

    if config is None:
        config = cfg
        config.merge_from_file("config/evaluation/eval_hnu.yaml")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="config/evaluation/eval_hnu.yaml")
    parser.add_argument('--datapath', default='', help='path to dataset directory')
    parser.add_argument('--weights', default="DEVO.pth")
    parser.add_argument('--val_split', type=str, default="splits/hnu/hnu_val.txt")
