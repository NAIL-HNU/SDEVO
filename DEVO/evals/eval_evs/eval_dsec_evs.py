...
import sys
sys.path.insert(0, '/home/njk/ros/SDEVO_no_speedup/src/sdevo/DEVO')

from utils.load_utils import load_gt_us, hnu_evs_iterator, get_file_names
from utils.eval_utils import assert_eval_config, run_voxel_hnu, run_voxel
...
def evaluate(config, args, net, train_step=None, datapath="", split_file=None,
    ...
    if config is None:
        config = cfg
        config.merge_from_file("config/evaluation/eval_dsec.yaml")
    ...
    for i, scene in enumerate(scenes):
        ...
        for trial in range(trials):
            ...
            # run the slam system
            traj_est, tstamps, flowdata = run_voxel_hnu(datapath_val, config, net, viz=False, 
                                          iterator=get_file_names(datapath_val, skip=True), # optionally pass DELTA_MS
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="config/evaluation/eval_dsec.yaml")
    parser.add_argument('--datapath', default='', help='path to dataset directory')
    parser.add_argument('--weights', default="DEVO.pth")
    parser.add_argument('--val_split', type=str, default="splits/dsec/dsec_val.txt")
    ...
    cfg.save_traj_dir = "/home/njk/ros/SDEVO_no_speedup/src/sdevo/DEVO/output/poses_tum_format.txt"
    cfg.baseline = 0.599
