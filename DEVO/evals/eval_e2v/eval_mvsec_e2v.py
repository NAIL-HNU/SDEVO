...
from utils.load_utils import load_mvsec_traj, video_iterator
...
def evaluate(config, args, net, train_step=None, datapath="", split_file=None,
    ...
    if config is None:
        config = cfg
        config.merge_from_file("config/dataset_configs/default_rgb.yaml")
    ...
    for i, scene in enumerate(scenes):
        ...
        for trial in range(trials):
            ...
            intrinsics = np.loadtxt(os.path.join(datapath_val, f"calib_undist_{side}.txt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="config/dataset_configs/default_rgb.yaml") # default_rgb_kf5
    ...
    print("Running eval_MVSEC.py with config...")
    print(cfg)
