...
from event_utils import EventSlicer, to_voxel_grid, RemoveHotPixelsVoxel
from viz_utils import visualize_voxel, visualize_N_voxels, render
from transform_utils import transform_rescale
import pickle
...
def get_ecd_data_low_memory(tss_imgs_us, evs_file, intrinsics, rectify_map, DELTA_MS=None, H=180, W=240, return_dict=None):
    print(f"Delta {DELTA_MS} ms in ", evs_file[0])
    data_list = []
    # evs = np.asarray(np.loadtxt(evs_file[0], delimiter=" ")) # (N, 4) with [ts_usecs, x, y, p]
    
    # Track the current reading position in the event file
    current_pos = 0

    # Open the file and process events iteratively
    with open(evs_file[0], 'r') as f:
        for (ts_idx, ts_us) in enumerate(tss_imgs_us):
            ...
            print(f"耗时0: {elapsed_time:.2f} 秒")

            # Read lines until reaching the desired time window
            while True:
                line = f.readline()
                if not line:
                    break

                event = np.fromstring(line, sep=' ')
                if event[0] >= t1_us:
                    break
                if t0_us <= event[0] < t1_us:
                    evs_batch.append(event)

            # Update current position
            current_pos = f.tell()
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"耗时1: {elapsed_time:.2f} 秒")

            if len(evs_batch) == 0:
                print(f"no events in range {ts_us * 1e-3} - {tss_imgs_us[ts_idx + 1] * 1e-3} milliseconds")
                continue

            evs_batch = np.array(evs_batch)
            ...
            with open("/media/njk/njk/data_for_devo/dsec/04b/test.pkl", 'wb') as f1:
                pickle.dump(data_list, f1)
            print(f"Saved data to /media/njk/njk/data_for_devo/dsec/04b/test.pkl")

            with open("/media/njk/njk/data_for_devo/dsec/04b/test.pkl", 'rb') as f1:
...
def hnu_evs_iterator(scenedir, side="left", stride=1, dT_ms=None, H=180, W=240, timing=False, parallel=False, cors=8):
    if timing:
        t0 = torch.cuda.Event(enable_timing=True)
        t1 = torch.cuda.Event(enable_timing=True)
        t0.record()
    ...
    tss_file = os.path.join(scenedir, f"tss_imgs_us_{side}.txt")
    tss_imgs_us = sorted(np.loadtxt(tss_file))
    print("排序后数据类型:", type(tss_imgs_us))
    if isinstance(tss_imgs_us, list):
        # 如果是列表，使用列表推导
        tss_imgs_us = [ts * 1e6 for ts in tss_imgs_us]
    elif isinstance(tss_imgs_us, np.ndarray):
        # 如果是NumPy数组，直接进行向量化计算
        tss_imgs_us *= 1e6
    else:
        # 其他情况，可以抛出异常或者进行特别处理
        raise TypeError("tss_imgs_us 必须是列表或NumPy数组")
    if dT_ms is None:
        dT_ms = np.diff(tss_imgs_us).mean()/1e3   
    assert dT_ms > 3 and dT_ms < 1000
    tss_imgs_us = tss_imgs_us[::stride]
    
    data_list = get_ecd_data(tss_imgs_us, evs_file, intrinsics, rectify_map, dT_ms, H, W)
...
def get_file_names(file_path, skip=False):
    file_paths = []
    with open(file_path + "/voxels_left/voxel_time.txt", 'r') as index_file:
        for line in index_file:
            filename = line.strip()  # 去除换行符
            filename1 = os.path.join(file_path, "voxels_left/" + filename + ".pkl")
            filename2 = os.path.join(file_path, "voxels_right/" + filename + ".pkl")
            file_paths.append([filename1, filename2])
            if skip:
                skip = False
                continue
    return file_paths
