...
@torch.no_grad()
def run_voxel_hnu(voxeldir, cfg, network, viz=False, iterator=None, timing=False, H=480, W=640, viz_flow=False, scale=1.0, **kwargs): 
    slam = DEVO(cfg, network, evs=True, ht=H, wd=W, viz=viz, viz_flow=viz_flow, **kwargs)
    
    for i, file_paths in enumerate(iterator):
        data_list = []
        ...
        with open(file_paths[1], 'rb') as f:
            data = pickle.load(f)
            data_list.append(data)
        voxel, intrinsics, t = data_list[0]
        voxel2, intrinsics2, t2 = data_list[1]
        intrinsics = torch.from_numpy(intrinsics)
        intrinsics = intrinsics.to(torch.float16).cuda()
        voxel = voxel.to(torch.float16).cuda()
        intrinsics2 = torch.from_numpy(intrinsics2)
        intrinsics2 = intrinsics2.to(torch.float16).cuda()
        voxel2 = voxel2.to(torch.float16).cuda()
        if timing and i == 0:
        ...
        if viz: 
            # import matplotlib.pyplot as plt
            # plt.switch_backend('Qt5Agg')
            visualize_voxel(voxel.detach().cpu(), save=True, folder="/home/njk/ros/SDEVO_no_speedup/src/sdevo/DEVO/results/voxels")
        
        with Timer("DEVO", enabled=timing):
            slam(t, voxel, intrinsics, voxel2, intrinsics2, scale=scale)
